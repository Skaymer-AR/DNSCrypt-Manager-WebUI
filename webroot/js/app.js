...
/* ===================== v0.3 WebUI: service-controls + transportes ===================== */
...
var V030 = (function () {
  ...
  function badge(stateKey, stateVal) {
    var b = el('span', 'v-badge v-' + stateVal); b.textContent = t(stateKey);
    // señal accesible además del color: icono textual
  ...
  /* ---- Service controls ---- */
  async function renderServiceControls() {
    var box = document.getElementById('scList'); if (!box) return;
    if (!DCM.cliResolved()) { try { await DCM.resolveCli(); } catch (e) {} }
    box.textContent = t('common.loading');
    var r;
    try { r = await DCM.runServiceControlListJson(); } catch (e) { box.textContent = t('common.error'); return; }
    if (r && r.errno && r.errno !== 0 && !(r.stdout || '').length) {
      box.textContent = ''; box.appendChild(el('div', 'v-err', (r.stderr || t('common.error')))); return;
    }
    var blocks = DCM.parseKvBlocks((r && r.stdout) ? r.stdout : '');
    box.textContent = '';
    if (!blocks.length) { box.appendChild(el('div', 'v-muted', '(—)')); return; }
    blocks.forEach(function (c) {
      var card = el('div', 'v-card');
      var head = el('div', 'v-card-h');
      head.appendChild(el('span', 'v-name', c.name || c.id));
      head.appendChild(badge('sc.st.' + (c.state || 'unknown'), c.state || 'unknown'));
      card.appendChild(head);
      card.appendChild(el('div', 'v-id', c.id));
      // filas de datos
      var grid = el('div', 'v-grid');
      function row(lblKey, val) { var r2 = el('div', 'v-row'); r2.appendChild(el('span', 'v-k', t(lblKey))); r2.appendChild(el('span', 'v-v', val == null ? '-' : String(val))); grid.appendChild(r2); }
      row('sc.col.requested', c.requested_mode);
      row('sc.col.applied', (c.domains_in_blocked || '0/0') + '  (' + t('tp.col.evidence').toLowerCase() + ')');
      row('sc.col.domains', c.domains);
      if (c.allowlist_conflict === 'yes') { var w = el('div', 'v-conflict'); w.textContent = '⚠ ' + t('sc.conflict'); card.appendChild(w); }
      card.appendChild(grid);
      // selector de duración + botones
      var ctr = el('div', 'v-actions');
      var sel = document.createElement('select'); sel.className = 'v-sel';
      MODES.forEach(function (m) { var o = document.createElement('option'); o.value = m; o.textContent = t('sc.mode.' + m); if (m === (c.requested_mode || 'off')) o.selected = true; sel.appendChild(o); });
      // por defecto seguro al encender sin elegir: until_reboot (solo visual)
      ctr.appendChild(sel);
      var apply = el('button', 'small', t('sc.btn.apply'));
      apply.addEventListener('click', function () {
        if (busy) return;
        var mode = sel.value; if (mode === 'off') mode = 'until_reboot'; // opción segura visual
        scApply(c.id, mode);
      });
      var off = el('button', 'small', t('sc.btn.off'));
      off.addEventListener('click', function () { if (!busy) scApply(c.id, 'off'); });
      var ver = el('button', 'small', t('sc.btn.verify'));
      ver.addEventListener('click', function () { if (!busy) scVerify(c.id); });
      ctr.appendChild(apply); ctr.appendChild(off); ctr.appendChild(ver);
      card.appendChild(ctr);
      var res = el('div', 'v-result'); res.id = 'scres-' + c.id; card.appendChild(res);
      box.appendChild(card);
    });
  }
  async function scApply(id, mode) {
    if (!DCM.validMode(mode) || !/^[a-zA-Z0-9_]+$/.test(id)) { toast(t('common.error'), 'error'); return; }
    setBusy(true);
    try { var r = await DCM.runServiceControlSet(id, mode); var box = document.getElementById('scres-' + id); if (box) box.textContent = ((r && (r.stdout || r.stderr)) || '').split('\n')[0]; }
    catch (e) {} finally { setBusy(false); renderServiceControls(); }
  }
  async function scVerify(id) {
    setBusy(true);
    try { var r = await DCM.runServiceControlVerify(id); var box = document.getElementById('scres-' + id); if (box) box.textContent = ((r && (r.stdout || r.stderr)) || '').replace(/\n/g, ' '); }
    catch (e) {} finally { setBusy(false); }
  }

  /* ---- Transport cards (Anonymized / ODoH) ---- */
  ...
  async function renderTransportCard(kind) {
    var box = document.getElementById(kind === 'odoh' ? 'odohCard' : 'anonCard'); if (!box) return;
    if (!DCM.cliResolved()) { try { await DCM.resolveCli(); } catch (e) {} }
    var r;
    try { r = (kind === 'odoh') ? await DCM.runOdohStatus() : await DCM.runAnonymizedStatus(); } catch (e) { box.textContent = t('common.error'); return; }
    ...
    if (kind === 'odoh') { row('tp.col.target', kv.target); row('tp.col.lasttest', kv.enabled); row('tp.col.evidence', kv.supported); }
    ...
    // botones
    var ctr = el('div', 'v-actions');
    var test = el('button', 'small', t('tp.btn.test'));
    test.addEventListener('click', function () { if (!busy) tpTest(kind); });
    var dis = el('button', 'small', t('tp.btn.disable'));
    dis.addEventListener('click', function () { if (!busy) tpDisable(kind); });
    var roll = el('button', 'small', t('tp.btn.rollback'));
    roll.addEventListener('click', function () { if (!busy) tpRollback(kind); });
    var refr = el('button', 'small', t('tp.btn.refresh'));
    refr.addEventListener('click', function () { if (!busy) renderTransportCard(kind); });
  ...
  async function tpTest(kind) {
    setBusy(true);
    var out = document.getElementById('tpres-' + kind); if (out) out.textContent = t('tp.st.testing');
    try {
      var r = (kind === 'odoh') ? await DCM.runOdohTest('', '') : await DCM.runAnonymizedTest(
        (document.getElementById('anonResolver') || {}).value || 'cloudflare',
        (document.getElementById('anonRelays') || {}).value || 'anon-cs-fr');
      if (out) out.textContent = ((r && (r.stdout || r.stderr)) || '').replace(/\n/g, ' ');
    } catch (e) {} finally { setBusy(false); renderTransportCard(kind); }
  }
  async function tpDisable(kind) {
    setBusy(true);
    try { var r = (kind === 'odoh') ? await DCM.runOdohDisable() : await DCM.runAnonymizedDisable(); }
    catch (e) {} finally { setBusy(false); renderTransportCard(kind); }
  }
  async function tpRollback(kind) {
    if (typeof confirm === 'function' && !confirm(t('tp.confirm.apply'))) return;
    setBusy(true);
    try { await DCM.runTransportRollback(); } catch (e) {} finally { setBusy(false); renderTransportCard(kind); }
  }

  /* ---- polling por ruta + visibilidad ---- */
  function renderForRoute(route) {
    if (route === 'lists') { renderServiceControls(); }
    else if (route === 'dns') { renderTransportCard('anon'); renderTransportCard('odoh'); }
  }
  function startPoll(route) {
    stopPoll();
    if (route !== 'lists' && route !== 'dns') return;
    pollTimer = setInterval(function () { if (!document.hidden && !busy) renderForRoute(route); }, POLL_MS);
  }
  function stopPoll() { if (pollTimer) { clearInterval(pollTimer); pollTimer = null; } }
  function onRoute(route) { renderForRoute(route); startPoll(route); }
  document.addEventListener('visibilitychange', function () {
    if (document.hidden) stopPoll();
    else if (typeof Router !== 'undefined' && Router.current) startPoll(Router.current());
  });
  return { onRoute: onRoute, renderForRoute: renderForRoute, renderServiceControls: renderServiceControls, renderTransportCard: renderTransportCard };
})();
